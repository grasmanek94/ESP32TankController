...
bool RH_RF95::init()
{
    if (!RHSPIDriver::init())
    {
        if (Serial)
        {
            Serial.println("!RHSPIDriver::init()");
        }
        return false;
    }
    ...
    if (interruptNumber == NOT_AN_INTERRUPT)
    {
        if (Serial)
        {
            Serial.println("interruptNumber == NOT_AN_INTERRUPT");
        }
        return false;
    }
    ...
    uint8_t data = spiRead(RH_RF95_REG_01_OP_MODE);
    if (data != (RH_RF95_MODE_SLEEP | RH_RF95_LONG_RANGE_MODE))
    {
        if (Serial)
        {
            Serial.printf("spiRead(RH_RF95_REG_01_OP_MODE)[D:%d, H:%02x] != (RH_RF95_MODE_SLEEP | RH_RF95_LONG_RANGE_MODE)\r\n",data,data);
        }
    ...
    if (_myInterruptIndex == 0xff)
    {
        ...
        else
        {
            if (Serial)
            {
                Serial.println("Too many devices, not enough interrupt vectors (1)");
            }
    ...
    else
    {
        if (Serial)
        {
            Serial.println("Too many devices, not enough interrupt vectors (2)");
        }
        return false; // Too many devices, not enough interrupt vectors
    }
...
void RH_RF95::handleInterrupt()
{
    ...
    if (_mode == RHModeRx && ((irq_flags & (RH_RF95_RX_TIMEOUT | RH_RF95_PAYLOAD_CRC_ERROR)) | !(crc_present & RH_RF95_RX_PAYLOAD_CRC_IS_ON)))
